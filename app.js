...
function getPublicConfigFallback() {
  return {
    ...
    localDeliveryMga: 0,
    marginTiers: [{ maxUsd:500, percent:35 }, { maxUsd:1000, percent:30 }, { maxUsd:null, percent:28 }],
...
async function analyzeProduct() {
  ...
  setHint('analysisStatus', 'Analyse du produit et estimation du colis…');
  try {
...
function marginRate(itemUsd) { return itemUsd <= 500 ? .35 : itemUsd <= 1000 ? .30 : .28; }
function localQuote() {
  ...
  const floor500 = 500 * rate * .35;
  const floor1000 = 1000 * rate * .30;
  const continuity = item > 1000 ? floor1000 : item > 500 ? floor500 : 0;
